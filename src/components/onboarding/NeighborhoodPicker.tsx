...
import { OB_H2, OB_SECTION_VARIANTS } from "@/components/onboarding/stepStyles";
...
/**
 * Shared neighborhood picker used by onboarding step 3 and the search
 * location editor so both screens behave and look identical.
 */
export function NeighborhoodPicker({ cityConfig }: { cityConfig: CityConfig }) {
  ...
  const groups = useMemo(
    () => Object.entries(cityConfig.neighborhoodGroups),
    [cityConfig],
  );
  ...
  const presets = useMemo(() => getCityPresets(cityConfig.id), [cityConfig]);
