...
BEGIN
  ...
  -- The 3 free days start when setup is finished, never at checkout.
...
-- Safety valve: paid but never finished setup within 14 days -> cancel, so a
-- trial can never sit open-ended waiting for onboarding to complete.
CREATE OR REPLACE FUNCTION public.expire_unfinished_setup(_user_id uuid)
...
REVOKE ALL ON FUNCTION public.expire_unfinished_setup(uuid) FROM PUBLIC;
REVOKE ALL ON FUNCTION public.expire_unfinished_setup(uuid) FROM anon;
REVOKE ALL ON FUNCTION public.expire_unfinished_setup(uuid) FROM authenticated;
GRANT EXECUTE ON FUNCTION public.expire_unfinished_setup(uuid) TO service_role;